...
par <- rep(0, p)
set.seed(6325203)
n40_res_retel_f <- vector("numeric", length = S)
n40_res_retel_r <- vector("numeric", length = S)
n40_res_aetel <- vector("numeric", length = S)
n40_res_ael <- vector("numeric", length = S)
n40_res_bael <- vector("numeric", length = S)
for (i in seq_len(S)) {
  # Data
  x <- rmvnorm(n, sigma = diag(p))

  # RETEL_f
  n40_res_retel_f[i] <- -2 * retel(f, x, par,
    mu = colMeans(x),
    Sigma = sqrt(p) * cov(x),
    tau = log(n),
    type = "full"
  ...
  # RETEL_r
  n40_res_retel_r[i] <- -2 * retel(f, x, par,
    mu = colMeans(x),
    Sigma = sqrt(p) * cov(x),
    tau = log(n),
    type = "reduced"
  ...
  # AETEL
  n40_res_aetel[i] <- -2 * aetel(x, par)

  # AEL
  p_ael <- par - an * (colMeans(x) - par)
  n40_res_ael[i] <- el_mean(rbind(x, p_ael), par = par) |>
    chisq()
  ...
  n40_res_bael[i] <- el_mean(rbind(x, p1_bael, p2_bael), par = par) |>
    chisq()
}
...
cv <- qchisq(0.95, df = p)
set.seed(6325203)
n200_res_retel_f <- vector("numeric", length = S)
n200_res_retel_r <- vector("numeric", length = S)
n200_res_aetel <- vector("numeric", length = S)
n200_res_ael <- vector("numeric", length = S)
n200_res_bael <- vector("numeric", length = S)
for (i in seq_len(S)) {
  # Data
  x <- rmvnorm(n, sigma = diag(p))

  # RETEL_f
  n200_res_retel_f[i] <- -2 * retel(f, x, par,
    mu = colMeans(x),
    Sigma = sqrt(p) * cov(x),
    tau = log(n),
    type = "full"
  ...
  # RETEL_r
  n200_res_retel_r[i] <- -2 * retel(f, x, par,
    mu = colMeans(x),
    Sigma = sqrt(p) * cov(x),
    tau = log(n),
    type = "reduced"
  ...
  # AETEL
  n200_res_aetel[i] <- -2 * aetel(x, par)

  # AEL
  p_ael <- par - an * (colMeans(x) - par)
  n200_res_ael[i] <- el_mean(rbind(x, p_ael), par = par) |>
    chisq()
  ...
  p2_bael <- 2 * colMeans(x) - par +
    1.9 * u / sqrt(colSums(u * (solve(cov(x)) %*% u)))
  n200_res_bael[i] <- el_mean(rbind(x, p1_bael, p2_bael), par = par) |>
    chisq()
}
...
## 5. Plots
# n = 40 ----
p_n40_retel_f <- ggplot(
  data.frame(sample = n40_res_retel_f),
  aes(sample = n40_res_retel_f)
) +
...
p_n40_retel_r <- ggplot(
  data.frame(sample = n40_res_retel_r),
  aes(sample = n40_res_retel_r)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
p_n40_aetel <- ggplot(
  data.frame(sample = n40_res_aetel),
  aes(sample = n40_res_aetel)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
p_n40_ael <- ggplot(
  data.frame(sample = n40_res_ael),
  aes(sample = n40_res_ael)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
p_n40_bael <- ggplot(
  data.frame(sample = n40_res_bael),
  aes(sample = n40_res_bael)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
# n = 200 ----
p_n200_retel_f <- ggplot(
  data.frame(sample = n200_res_retel_f),
  aes(sample = n200_res_retel_f)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
p_n200_retel_r <- ggplot(
  data.frame(sample = n200_res_retel_r),
  aes(sample = n200_res_retel_r)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
p_n200_aetel <- ggplot(
  data.frame(sample = n200_res_aetel),
  aes(sample = n200_res_aetel)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
p_n200_ael <- ggplot(
  data.frame(sample = n200_res_ael),
  aes(sample = n200_res_ael)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
p_n200_bael <- ggplot(
  data.frame(sample = n200_res_bael),
  aes(sample = n200_res_bael)
) +
  stat_qq(distribution = qchisq, dparams = list(df = p), size = 0.5) +
  stat_qq_line(distribution = qchisq, dparams = list(df = p)) +
  theme(
...
plots <- plot_grid(
  p_n40_retel_f, p_n40_retel_r, p_n40_aetel, p_n40_ael, p_n40_bael,
  p_n100_retel_f, p_n100_retel_r, p_n100_aetel, p_n100_ael, p_n100_bael,
  p_n200_retel_f, p_n200_retel_r, p_n200_aetel, p_n200_ael, p_n200_bael,
  nrow = 3L, align = "v"
)
