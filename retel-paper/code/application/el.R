...
suppressMessages(i_am("code/application/el.R"))
library(melt)
library(mvtnorm)
library(retel)
...
p <- ncol(x)
# Optimization
ctrl <- el_control(th = 1000, nthreads = 1L, tol = 1e-04, tol_l = 1e-04)
# MCMC parameters
B <- 250000L
...
log_posterior_unnormalized <- function(theta, b, s2) {
  g <- cbind(y - theta, (y - theta)^2L - 1)
  eval <- el_eval(g, control = ctrl)
  if (isFALSE(eval$optim$convergence)) {
    loglr <- -10000
  } else {
    loglr <- eval$loglr
  }
  log_pd(theta, b, s2) + loglr
}
# MCMC
mcmc_fn <- function(B) {
